---
title: "PS811 Portfolio - The Impact of Formal Property Rights on Women's Tenure in Africa"
...
world %<>% full_join(landata)

```

### Map of Share of Labor Force Employed in Agriculture 
```{r map of share of labor force in agriculture}
...
```{r map of women tenure security}
...
### Map of Strength of Formal Property Rights
```{r map formal rights}
...
$y_i = \beta_0 + \beta_1*FormalPropertyRights_i + \beta_2*Corruption + \beta_3*ArableLand + \beta_4*LaborAgriculture + \beta_5*GDP + \beta_6*RuralPop +\epsilon_i$
...
kenya %<>% 
  rename(country = Q3) %>%
  rename(gender = Q21_Gender) %>%
  rename(formaldocs = Q55_any_formal) %>%
  rename(tenuresecurity = Q45_security)

myvariables <- c("country", "gender", "formaldocs", "tenuresecurity")

kenyadata <- kenya[myvariables]

m2 <- glm(factor(tenuresecurity) ~ formaldocs*gender, data = kenyadata, family = "binomial")
